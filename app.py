...
BOR_LOCK = "BOR"
BOR_TTL = 600  # Lock expiration time (10 minutes)
...
def _get_chunk_size(num_roles: int) -> int:
    chunk_size_cfg = repository._settings.get_fresh("BOR_CHUNK_SIZE", 500)
...
def _update_snapshot_timestamp(*args) -> Dict[str, Any]:
    ...
    logging.info(
        f"Time parsing _update_snapshot_timestamp: {time.time() - start_time} seconds"
    )
    ...
    logging.info(
        f"Time updating _update_snapshot_timestamp: {time.time() - start_time} seconds"
    )
    ...
    try:
        with repository._redis.lock("LOCK_TARGETS", repository._timeout):
            ...
            if chunk_size == 0:
                ...
                # Run updates using chunks which improves the performance
                roles_updated = _update_online_role.chunks(
                    zip(roles), chunk_size
                ).group()
                logging.debug(
                    f"Tasks: {len(roles_updated)} | Chunk size: {chunk_size}"
                )
                c = chain(
                    roles_updated,
